...
export function useHubTelemetry(enabled: boolean): TelemetryController {
  ...
  const [manualAddresses, setManualAddresses] = useState<string[]>(initialManualHubAddresses);
  ...
  useEffect(() => { localStorage.setItem(HUBS_KEY, JSON.stringify(manualAddresses)); }, [manualAddresses]);
