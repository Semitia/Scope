...
import type { ValueBarPanelDefinition } from '../../src/panelTypes';
...
function Fixture() {
  const [panel, setPanel] = useState<ValueBarPanelDefinition>({
    id: 'values', type: 'value-bar', title: 'Values',
  ...
  <div id="panel" style={{ width: 440, height: 600, containerType: 'inline-size' }}>
    <ValueBarPanel panel={panel}
      channels={channels} data={[[0], ...values.map(v => [v])]} latest={values}
      channelIndexes={new Map(channels.map((c, i) => [c.id, i]))} onChange={patch => setPanel(current => ({ ...current, ...patch }))} />
  </div>
  );
}
